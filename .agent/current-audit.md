...
**Audit timestamp:** `2026-07-08T18-51-55-04-00`

## Summary

`TheUnmappedHouse` remains a compact fixed-camera anime point-and-click horror prototype.

The accessible `LuminaryLabs-Publish` organization repo list was compared against central `LuminaryLabs-Dev/LuminaryLabs` tracking and sampled root `.agent/START_HERE.md` state. No checked non-Cavalry repo was fully new, absent from tracking, undocumented, recently added but undocumented, or missing sampled root `.agent` state.

`TheUnmappedHouse` was selected as the oldest eligible fallback. This pass keeps the story-authority direction but narrows the next implementation into a preflight/result fixture contract and a Stage projection readback gate.
...
src/aspect-frame.js:
  owns canonical 1920x1080 design frame math and letterbox/pillarbox application.
...
The render surface should stay stable. `StageKit` already owns fixed 16:9 stage display, shader material, post-process, hotspot picking, and scene loading.

The blocker is still source authority in `src/game.js`: story command validation, scene/source preflight, result statuses, reason codes, route/completion decisions, save intent, interlude intent, reset intent, and diagnostics remain mixed into DOM handlers and mutable module state.

The sharper next cut is not broad extraction. It is a pure `StoryPreflight` plus `StoryCommandResult` fixture path that can prove each story transition before the browser host consumes projections.
