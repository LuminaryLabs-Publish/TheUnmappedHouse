...
**Audit timestamp:** `2026-07-09T01-40-49-04-00`

## Summary

`TheUnmappedHouse` remains a compact fixed-camera anime point-and-click horror prototype.

The accessible `LuminaryLabs-Publish` repo list was compared against central `LuminaryLabs-Dev/LuminaryLabs` tracking and sampled repo-local `.agent/START_HERE.md` state.

No checked non-Cavalry repo was fully new, absent from tracking, recently added but undocumented, missing sampled root `.agent` state, or otherwise undocumented.

`TheUnmappedHouse` was selected as the oldest eligible central-ledger fallback. This pass preserves the story-authority direction and sharpens the next implementation into a story adapter readback fixture plus central-ledger freshness gate.
...
The render surface should remain stable.

`StageKit` already owns the fixed 16:9 display, shader material, post-process, hotspot picking, hover labels, and scene descriptor loading.

The highest-value next pass is not story expansion, visual rewrite, audio, inventory, or renderer extraction.

It is a pure story-authority layer with `StorySourceManifest`, command/result records, projection objects, browser-adapter plans, adapter readback diagnostics, central-ledger freshness rows, and additive `GameHost` diagnostics that make `src/game.js` a consumer instead of the source of truth.
