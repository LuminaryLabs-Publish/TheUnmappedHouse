...
- There are no stable reason codes for repeated hotspot, unknown hotspot, incomplete continue, prototype complete, invalid command, malformed save, duplicate descriptor, ungrantable required clue, or browser-adapter readback.
...
- There is no command journal, route journal, save journal, preflight journal, adapter journal, readback journal, or fixture summary row shape.
...
- Current `debug` JSON does not include latest command result, preflight status, result reason, event records, fixture status, save projection, interlude projection, stage projection, stage snapshot, adapter plan, adapter readback, or source validation status.
...
- Before this pass, the central `LuminaryLabs-Dev/LuminaryLabs` repo ledger still pointed to an older `TheUnmappedHouse` tracker than the repo-local `.agent/START_HERE.md` readback.
- The central ledger needs to stay aligned to latest tracker, turn-ledger, architecture, render, interaction, gameplay, story-authority, deploy, and kit-registry paths after every repo-local documentation pass.
